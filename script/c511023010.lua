...
function c511023010.initial_effect(c)
	...
	e1:SetValue(c511023010.splimit)
	c:RegisterEffect(e1)
	--special summon rule
	...
	e2:SetCondition(c511023010.spcon)
	e2:SetOperation(c511023010.spop)
	c:RegisterEffect(e2)
	--return
...
c511023010.material_setcode=0x8
function c511023010.splimit(e,se,sp,st)
	return not e:GetHandler():IsLocation(LOCATION_EXTRA)
end
function c511023010.spfilter(c,code)
	return c:IsAbleToDeckOrExtraAsCost() and c:IsFusionCode(code)
end
function c511023010.spcon(e,c)
	if c==nil then return true end
	local tp=c:GetControler()
	local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
	if ft<-1 then return false end
	local g1=Duel.GetMatchingGroup(c511023010.spfilter,tp,LOCATION_ONFIELD,0,nil,89943723)
	local g2=Duel.GetMatchingGroup(c511023010.spfilter,tp,LOCATION_ONFIELD,0,nil,17955766)
...
function c511023010.spop(e,tp,eg,ep,ev,re,r,rp,c)
	local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
	local g1=Duel.GetMatchingGroup(c511023010.spfilter,tp,LOCATION_ONFIELD,0,nil,89943723)
	local g2=Duel.GetMatchingGroup(c511023010.spfilter,tp,LOCATION_ONFIELD,0,nil,17955766)
	g1:Merge(g2)
	...
	for i=1,2 do
		...
		if i==1 then
			...
			if tc:IsFusionCode(89943723) then
				local sg=Duel.GetMatchingGroup(c511023010.spfilter,tp,LOCATION_ONFIELD,0,tc,17955766)
				g1:Merge(sg)
			end
			if tc:IsFusionCode(17955766) then
				local sg=Duel.GetMatchingGroup(c511023010.spfilter,tp,LOCATION_ONFIELD,0,tc,89943723)
				g1:Merge(sg)
			end
		end
	end
