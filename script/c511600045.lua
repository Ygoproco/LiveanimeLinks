...
function c511600045.initial_effect(c)
	...
	e2:SetCondition(c511600045.condition)
	e2:SetCost(c511600045.cost)
	e2:SetOperation(c511600045.operation)
	c:RegisterEffect(e2)
	--destroy
...
function c511600045.condition(e,tp,eg,ep,ev,re,r,rp)
...
function c511600045.cost(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c511600045.operation(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local tc=Duel.GetFirstTarget()
	if not tc:IsSSetable(true) or not c:IsRelateToEffect(e) then return end
	if tc:IsSSetable() and not Duel.IsExistingMatchingCard(c511600045.filter,tp,LOCATION_SZONE,LOCATION_SZONE,1,nil) then
		Duel.SSet(tp,tc)
		Duel.ConfirmCards(1-tp,tc)
		local fid=c:GetFieldID()
		tc:RegisterFlagEffect(511600045,RESET_EVENT+0x1fe0000,0,1,fid)
		local e1=Effect.CreateEffect(c)
