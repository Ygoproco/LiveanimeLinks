...
function c810000060.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:IsRelateToEffect(e) and tc:IsFaceup() then
		tc:RegisterFlagEffect(810000060,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1)
		local e1=Effect.CreateEffect(e:GetHandler())
		e1:SetType(EFFECT_TYPE_FIELD)
		e1:SetCode(EFFECT_DISABLE)
		e1:SetTargetRange(LOCATION_MZONE,LOCATION_MZONE)
		e1:SetTarget(c810000060.distg)
		...
		e2:SetCondition(c810000060.atkcon)
		e2:SetOperation(c810000060.atkop)
		e2:SetLabelObject(tc)
		e2:SetReset(RESET_PHASE+PHASE_END)
		Duel.RegisterEffect(e2,tp)
	end
end
function c810000060.distg(e,c)
	local bc=c:GetBattleTarget()
	local tc=e:GetLabelObject()
	return bc and bc==tc and tc:GetFlagEffect(810000060)~=0
end
function c810000060.atkcon(e,tp,eg,ep,ev,re,r,rp)
	local tc=e:GetLabelObject()
	local bc=tc:GetBattleTarget()
	return tc:GetFlagEffect(810000060)~=0 and bc and bc:GetDefense()>tc:GetAttack()
end
