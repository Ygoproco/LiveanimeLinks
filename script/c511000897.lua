...
function c511000897.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local g=Duel.SelectReleaseGroup(tp,c511000897.cfilter,1,1,nil,ft,tp)
	local tc=g:GetFirst()
...
function c511000897.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return tc and Duel.GetLocationCount(tp,LOCATION_MZONE)>-1 and tc:IsLocation(LOCATION_GRAVE) and tc:GetFlagEffect(511000897)>0
		and tc:IsType(TYPE_SYNCHRO) and tc:IsCanBeSpecialSummoned(e,0,tp,false,false) end
...
function c511000897.spop(e,tp,eg,ep,ev,re,r,rp,chk)
	local tc=Duel.GetFirstTarget()
	if e:GetHandler():IsRelateToEffect(e) and tc and tc:GetFlagEffect(511000897)>0 and tc:IsRelateToEffect(e) then
