...
function c511000209.filter1(c,e,tp)
	...
	return pg:GetCount()<=1 and c:IsFaceup() and c:IsSetCard(0x7f) 
		and Duel.IsExistingMatchingCard(c511000209.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,c,pg)
end
function c511000209.filter2(c,e,tp,mc,pg)
...
function c511000209.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c511000209.filter2,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,tc,pg)
	local sc=g:GetFirst()
	if sc then
