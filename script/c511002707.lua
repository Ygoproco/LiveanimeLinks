...
function c511002707.filter(c,fusc)
	return bit.band(c:GetReason(),0x40008)==0x40008 and c:GetReasonCard()==fusc and c:IsType(TYPE_SYNCHRO) 
		and c:IsAbleToRemove() and aux.SpElimFilter(c,true) and c:IsRace(RACE_DRAGON)
end
function c511002707.efftg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE+LOCATION_GRAVE) and chkc:IsControler(tp)
		and c511002707.filter(chkc,e:GetHandler()) end
	if chk==0 then return Duel.IsExistingTarget(c511002707.filter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,1,nil,e:GetHandler()) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectTarget(tp,c511002707.filter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,1,1,nil,e:GetHandler())
