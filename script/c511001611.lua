...
function c511001611.mfilter(c)
	return c.xyz_number and type(c.xyz_number) == "number"
end
function c511001611.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		local mg=Duel.GetMatchingGroup(c511001611.mfilter,tp,LOCATION_EXTRA,0,nil)
...
function c511001611.activate(e,tp,eg,ep,ev,re,r,rp)
	local pg=aux.GetMustBeMaterialGroup(tp,Group.CreateGroup(),tp,nil,nil,REASON_XYZ)
	local mg,mg0=Duel.GetMatchingGroup(c511001611.mfilter,tp,LOCATION_EXTRA,0,nil):Split(function(c)return c.xyz_number>0 end,nil)
