...
function c100100015.cfilter(c,e,tp,ft)
	local lv=c:GetLevel()
	return lv>0 and (ft>0 or (c:IsControler(tp) and c:GetSequence()<5)) and (c:IsControler(tp) or c:IsFaceup()) 
		and Duel.IsExistingMatchingCard(c100100015.spfilter,tp,LOCATION_DECK,0,1,nil,lv+1,e,tp)
end
...
function c100100015.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		...
		e:SetLabel(0)
		return Duel.CheckReleaseGroupCost(tp,c100100015.cfilter,1,false,nil,nil,e,tp,ft)
	end
	local rg=Duel.SelectReleaseGroupCost(tp,c100100015.cfilter,1,1,false,nil,nil,e,tp,ft)
