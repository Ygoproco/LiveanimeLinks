...
function c511005634.initial_effect(c)
	--coin
	...
	e1:SetTarget(c511005634.cointg)
	e1:SetOperation(c511005634.coinop)
	c:RegisterEffect(e1)
	local e2=e1:Clone()
...
function c511005634.cointg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_COIN,nil,0,tp,1)
end
function c511005634.coinop(e,tp,eg,ep,ev,re,r,rp)
	...
	c511005634.arcanareg(c,res)
end
function c511005634.arcanareg(c,coin)
	--coin effect
	...
	e0:SetCountLimit(1)
	e0:SetCondition(c511005634.descon)
	e0:SetTarget(c511005634.destg555)
	e0:SetOperation(c511005634.desop555)
	...
	e1:SetCountLimit(1)
	e1:SetCondition(c511005634.descon2)
	e1:SetTarget(c511005634.destg2)
	e1:SetOperation(c511005634.desop2)
	e1:SetReset(RESET_EVENT+0x1fe0000)
	c:RegisterEffect(e1)
	c:RegisterFlagEffect(36690018,RESET_EVENT+0x1fe0000,EFFECT_FLAG_CLIENT_HINT,1,coin,63-coin)
end

function c511005634.descon(e,tp,eg,ep,ev,re,r,rp)
	return tp==Duel.GetTurnPlayer()
end


function c511005634.filter555(c)
	return c:IsDestructable() 
end

function c511005634.destg555(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	--self
local heads=e:GetHandler():GetFlagEffectLabel(36690018)==1
if heads then 		
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c511005634.filter(chkc) end
	local c=e:GetHandler()
	if chk==0 then return true end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,c511005634.filter555,tp,LOCATION_MZONE,0,1,1,nil)
	Duel.SetTargetPlayer(tp)
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,g,1,0,0)
	Duel.SetOperationInfo(0,CATEGORY_DAMAGE,nil,0,tp,0)
	Duel.RegisterFlagEffect(tp,511005634,RESET_EVENT+0x1ff0000+RESET_PHASE+PHASE_END+RESET_SELF_TURN,0,1)
else
--opponents
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1-tp) and c511005634.filter(chkc) end
	local c=e:GetHandler()
	if chk==0 then return Duel.GetFieldGroupCount(c:GetControler(),0,LOCATION_MZONE)>0  end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,c511005634.filter555,tp,0,LOCATION_MZONE,1,1,nil)
...
function c511005634.desop555(e,tp,eg,ep,ev,re,r,rp)
...
function c511005634.descon2(e,tp,eg,ep,ev,re,r,rp)
	return tp==Duel.GetTurnPlayer() and  Duel.GetFlagEffect(tp,511005634)==0
end

function c511005634.destg2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local heads=e:GetHandler():GetFlagEffectLabel(36690018)==1
	if chkc then 
...
function c511005634.desop2(e,tp,eg,ep,ev,re,r,rp)
