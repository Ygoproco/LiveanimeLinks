...
--  By Shad3
--fixed by MLD
function c511005076.initial_effect(c)
	--Activate
	...
	e1:SetCategory(CATEGORY_DRAW)
	e1:SetCondition(c511005076.condition)
	e1:SetTarget(c511005076.target)
	e1:SetOperation(c511005076.activate)
	c:RegisterEffect(e1)
end
function c511005076.condition(e,tp,eg,ep,ev,re,r,rp)
	local tc=eg:GetFirst()
	return tc:IsRelateToBattle() and tc:IsControler(tp) and tc:IsStatus(STATUS_OPPO_BATTLE)
end
function c511005076.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsPlayerCanDraw(tp,1) end
	Duel.SetTargetPlayer(tp)
	Duel.SetTargetParam(1)
	Duel.SetOperationInfo(0,CATEGORY_DRAW,nil,0,tp,1)
end
function c511005076.activate(e,tp,eg,ep,ev,re,r,rp)
