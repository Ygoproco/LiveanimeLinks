...
function c511600099.initial_effect(c)
	...
	e3:SetOperation(c511600099.tkop)
	c:RegisterEffect(e3)
end
...
function c511600099.tkop2(e,tp,eg,ep,ev,re,r,rp)
	if not Duel.IsPlayerCanSpecialSummonMonster(tp,511600099+100,0,0x4011,0,0,1,RACE_CYBERSE,ATTRIBUTE_DARK) then return end
	...
	while ct>0 and ctn do
		local token=Duel.CreateToken(tp,511600099+100)
		Duel.SpecialSummonStep(token,0,tp,tp,false,false,POS_FACEUP)
		e:GetLabelObject():GetLabelObject():AddCard(token)
		ct=ct-1
		if ct<=0 or not Duel.SelectYesNo(tp,aux.Stringid(511600099,0)) then ctn=false end
	end
