...
function c100100114.cfilter(c,e,tp,ft)
	local lv=c:GetLevel()
	return lv>0 and (ft>0 or (c:IsControler(tp) and c:GetSequence()<5)) and (c:IsControler(tp) or c:IsFaceup()) 
		and Duel.IsExistingMatchingCard(c100100114.filter,tp,LOCATION_HAND,0,1,nil,lv,e,tp)
end
...
function c100100114.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		...
		e:SetLabel(0)
		return ft>-1 and Duel.CheckReleaseGroupCost(tp,c100100114.cfilter,1,false,nil,nil,e,tp,ft)
	end
	local rg=Duel.SelectReleaseGroupCost(tp,c100100114.cfilter,1,1,false,nil,nil,e,tp,ft)
