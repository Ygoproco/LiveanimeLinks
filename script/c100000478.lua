...
function c100000478.filter(c,e,tp)
	return c:IsFaceup() and c:IsLevel(4) 
		and Duel.IsExistingMatchingCard(c100000478.spfilter,tp,LOCATION_MZONE,0,2,nil,e,tp,c:GetRace())
end
...
function c100000478.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c100000478.filter(chkc,e,tp) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>1 and not Duel.IsPlayerAffectedByEffect(tp,59822133) 
		and Duel.IsExistingTarget(c100000478.filter,tp,LOCATION_MZONE,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	Duel.SelectTarget(tp,c100000478.filter,tp,LOCATION_MZONE,0,1,1,nil,e,tp)
...
function c100000478.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.GetMatchingGroup(c100000478.spfilter,tp,LOCATION_HAND+LOCATION_DECK,0,nil,e,tp,tc:GetRace())
	local tc=Duel.GetFirstTarget()
	if tc and tc:IsRelateToEffect(e) and tc:IsFaceup() and g:GetCount()>1 then
