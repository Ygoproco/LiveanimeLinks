...
function c513000163.arcanareg(c,coin)
	...
	e0:SetTarget(c513000163.destg555)
	e0:SetOperation(c513000163.desop555)
	...
	e1:SetCountLimit(1)
	e1:SetCondition(c513000163.descon2)
	e1:SetTarget(c513000163.destg2)
	e1:SetOperation(c513000163.desop2)
...
function c513000163.filter555(c)
	return c:IsDestructable() 
end

function c513000163.destg555(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	--self
local heads=e:GetHandler():GetFlagEffectLabel(36690018)==1
if heads then 		
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c513000163.filter(chkc) end
	local c=e:GetHandler()
	if chk==0 then return true end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,c513000163.filter555,tp,LOCATION_MZONE,0,1,1,nil)
...
--opponents
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1-tp) and c513000163.filter(chkc) end
	local c=e:GetHandler()
	if chk==0 then return Duel.GetFieldGroupCount(c:GetControler(),0,LOCATION_MZONE)>0  end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,c513000163.filter555,tp,0,LOCATION_MZONE,1,1,nil)
...
function c513000163.desop555(e,tp,eg,ep,ev,re,r,rp)
...
function c513000163.descon2(e,tp,eg,ep,ev,re,r,rp)
	return tp==Duel.GetTurnPlayer() and  Duel.GetFlagEffect(tp,511005634)==0
end

function c513000163.destg2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local heads=e:GetHandler():GetFlagEffectLabel(36690018)==1
	if chkc then 
...
function c513000163.desop2(e,tp,eg,ep,ev,re,r,rp)
