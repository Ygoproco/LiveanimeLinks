...
function s.etarget(e,c)
	return c:IsFaceup() and c:GetFlagEffect(id)~=0 and c:IsSetCard(SET_MARINCESS) and c:IsType(TYPE_LINK) and c:IsSummonType(SUMMON_TYPE_LINK)
end
function s.efilter(e,re)
	return e:GetOwnerPlayer()~=re:GetOwnerPlayer()
end
function s.cfilter(c)
	return c:IsFaceup() and c:IsSetCard(SET_MARINCESS) and c:IsType(TYPE_LINK) and c:GetSequence()>4
end
function s.condition(e,tp,eg,ep,ev,re,r,rp)
	return eg:IsExists(s.cfilter,1,nil)
end
function s.eqfilter(c)
	return c:IsSetCard(SET_MARINCESS) and c:IsType(TYPE_LINK) and not c:IsForbidden()
end
