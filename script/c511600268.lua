...
function s.initial_effect(c)
	...
	e2:SetTarget(aux.TargetBoolFunction(Card.IsSetCard,0x582))
	e2:SetValue(200)
	c:RegisterEffect(e2)
	--atk2
...
function s.atkfilter(c)
	return c:GetOriginalType()&(TYPE_LINK+TYPE_MONSTER)==(TYPE_LINK+TYPE_MONSTER) and c:IsSetCard(0x582)
end
...
function s.etarget(e,c)
	return c:IsFaceup() and c:GetFlagEffect(id)~=0 and c:IsSetCard(0x582) and c:IsType(TYPE_LINK) and c:IsSummonType(SUMMON_TYPE_LINK)
end
function s.efilter(e,re)
	return e:GetOwnerPlayer()~=re:GetOwnerPlayer()
end
function s.cfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x582) and c:IsType(TYPE_LINK) and c:GetSequence()>4
end
function s.condition(e,tp,eg,ep,ev,re,r,rp)
	return eg:IsExists(s.cfilter,1,nil)
end
function s.eqfilter(c)
	return c:IsSetCard(0x582) and c:IsType(TYPE_LINK) and not c:IsForbidden()
end
