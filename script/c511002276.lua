...
function c511002276.initial_effect(c)
	...
	e2:SetCost(c511002276.atcost)
	e2:SetOperation(c511002276.atop)
	c:RegisterEffect(e2)
end
...
function c511002276.atcost(e,c,tp)
	local g,da=e:GetHandler():GetAttackableTarget()
	return e:GetHandler():GetEffectCount(EFFECT_DIRECT_ATTACK)>1 or g:GetCount()>0 or Duel.CheckReleaseGroup(tp,nil,1,e:GetHandler()) 
		or Duel.GetFieldGroupCount(tp,0,LOCATION_MZONE)==0
end
function c511002276.atop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.IsAttackCostPaid()~=2 and c:IsLocation(LOCATION_MZONE) then
		...
		if c:GetEffectCount(EFFECT_DIRECT_ATTACK)>1 then
			...
			if g:GetCount()==0 or Duel.SelectYesNo(tp,500) then
				Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RELEASE)
				local tc=Duel.GetReleaseGroup(tp):Filter(aux.TRUE,c):SelectUnselect(Group.CreateGroup(),tp,Duel.IsAttackCostPaid()==0, Duel.IsAttackCostPaid()==0)
				if tc then
					Duel.Release(tc,REASON_COST)
