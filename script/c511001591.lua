...
function c511001591.operation(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(c511001591.filter,tp,0,LOCATION_MZONE,nil)
	local tg=g:GetMaxGroup(Card.GetLevel)
	if g:GetCount()==0 or not tg then return end
	if Duel.Destroy(tg,REASON_EFFECT)>0 and tg:IsExists(c511001591.filter2,1,nil,e,tp) and Duel.GetLocationCount(1-tp,LOCATION_MZONE)>0 then
