...
os = require('os')
function c511001940.initial_effect(c)
	--flip
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(51101940,1))
	e1:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_FLIP)
	e1:SetOperation(c511001940.operation)
	c:RegisterEffect(e1)
end
c511001940.collection={
...
function c511001940.operation(e,tp,eg,ep,ev,re,r,rp)
	local endtime=0
	local check=true
	local start=os.time()
	...
	if endtime>10 or not c511001940.collection[ac1] then
		check=false
	else
		Duel.Hint(HINT_SELECTMSG,1-tp,aux.Stringid(4003,9))
		local ac2=Duel.AnnounceCard(1-tp)
		endtime=os.time()-start
		if endtime>10 or not c511001940.collection[ac1] or ac1==ac2 then
			check=false
		else
			Duel.Hint(HINT_SELECTMSG,1-tp,aux.Stringid(4003,9))
			local ac3=Duel.AnnounceCard(1-tp)
			endtime=os.time()-start
			if endtime>10 or not c511001940.collection[ac1] or ac1==ac3 or ac3==ac2 then
				check=false
			end
		end
	end
