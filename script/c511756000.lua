...
function c511756000.atkcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local hun={}
	local hunatk=atk/100
	local oneatk=atk%100
	local one={}
	local p=1
	local start=0
	if hunatk>0 then
		for i=start,hunatk do 
			hun[p]=i p=p+1
		end
	end
	local hund=0
	if #hun>0 then
		Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(4008,0))
		hund=Duel.AnnounceNumber(tp,table.unpack(hun))
	end
	local hundatk=hund*100
	atk=atk-hundatk
	if atk>99 then oneatk=99 end
	if hund>0 then
		start=0
	else
		start=1
	...
	p=1
	if oneatk>0 then
		for i=start,oneatk do 
			one[p]=i p=p+1
		end
	end
	local oned=0
	if #one>0 then
		Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(4008,1))
		oned=Duel.AnnounceNumber(tp,table.unpack(one))
	end
	local finatk=hundatk+oned
	local e1=Effect.CreateEffect(c)
