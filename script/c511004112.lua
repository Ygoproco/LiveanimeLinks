...
function c511004112.cfilter(c)
	return c:IsSetCard(0x66) and c:IsType(TYPE_MONSTER)
end
function c511004112.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local g=eg:Filter(c511004112.cfilter,nil)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1-tp) and chkc:IsFaceup() end
	if chk==0 then return g:GetCount()==1 and Duel.IsExistingTarget(Card.IsFaceup,tp,0,LOCATION_MZONE,1,nil) end
	local tc=g:GetFirst()
