...
function c511000340.filter(c,ft)
...
function c511000340.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c511000340.filter(chkc,ft) end
	if chk==0 then return ft>-1 and Duel.IsExistingTarget(c511000340.filter,tp,LOCATION_MZONE,0,1,nil,ft) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RTOHAND)
	local g=Duel.SelectTarget(tp,c511000340.filter,tp,LOCATION_MZONE,0,1,1,nil,ft)
...
function c511000340.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:IsFaceup() and tc:IsRelateToEffect(e) then
		...
		if Duel.SendtoHand(tc,nil,REASON_EFFECT)>0 and tc:IsLocation(LOCATION_HAND)
			...
			local sc=Duel.SelectMatchingCard(tp,c511000340.spfilter,tp,LOCATION_HAND,0,1,1,nil,e,tp,lv):GetFirst()
