...
function c511600210.initial_effect(c)
	--selfdes
	...
	e0:SetCondition(c511600210.con)
	c:RegisterEffect(e0)
	--activate
	...
	e1:SetCondition(c511600210.condition)
	e1:SetTarget(c511600210.target)
	e1:SetOperation(c511600210.activate)
	c:RegisterEffect(e1)
	--cannot attack
	...
	e3:SetDescription(aux.Stringid(511600210,1))
	e3:SetCategory(CATEGORY_DISABLE+CATEGORY_DESTROY)
	e3:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_F)
	e3:SetProperty(EFFECT_FLAG_DELAY+EFFECT_FLAG_DAMAGE_STEP)
	e3:SetCode(EVENT_LEAVE_FIELD)
	e3:SetCondition(c511600210.descon)
	e3:SetTarget(c511600210.destg)
	e3:SetOperation(c511600210.desop)
	c:RegisterEffect(e3)
end
function c511600210.con(e)
	return Duel.GetFlagEffect(e:GetHandlerPlayer(),e:GetHandler():GetFieldID())==0
end
function c511600210.condition(e,tp,eg,ep,ev,re,r,rp)
	return ep~=tp and (aux.damcon1(e,0,eg,ep,ev,re,r,rp) or aux.damcon1(e,1,eg,ep,ev,re,r,rp))
end
function c511600210.target(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c511600210.activate(e,tp,eg,ep,ev,re,r,rp)
...
function c511600210.damval(e,re,val,r,rp,rc)
...
function c511600210.descon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
...
function c511600210.negfilter(c)
	return c:IsFaceup() and not c:IsDisabled()
end
function c511600210.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c511600210.negfilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil) end
	local g=Duel.GetMatchingGroup(c511600210.negfilter,tp,LOCATION_MZONE,LOCATION_MZONE,nil)
...
function c511600210.desop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local g=Duel.GetMatchingGroup(c511600210.negfilter,tp,LOCATION_MZONE,LOCATION_MZONE,nil)
