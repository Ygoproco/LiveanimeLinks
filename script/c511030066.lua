...
function s.initial_effect(c)
	...
	e0:SetTarget(Auxiliary.LinkTarget(s.matfilter,1,1,nil))
...
function s.linkcon(e,c)
	...
	local mg=g:Filter(Auxiliary.LConditionFilter,nil,s.matfilter,c,tp)
	local mustg=Auxiliary.GetMustBeMaterialGroup(tp,g,tp,c,mg,REASON_LINK)
	if mustg:IsExists(aux.NOT(Auxiliary.LConditionFilter),1,nil,s.matfilter,c,tp) then return false end
	local emt,tg=aux.GetExtraMaterials(tp,mustg+mg,c,SUMMON_TYPE_LINK)
	local sg=mustg
	return (mg+tg):Includes(mustg) and ((#sg>=1 and aux.LCheckGoal(tp,mustg,c,1,s.matfilter,nil,{})) or (mg+tg):IsExists(Auxiliary.LCheckRecursive,1,sg,tp,sg,(mg+tg),c,1,1,s.matfilter,nil,mg,emt))
end
