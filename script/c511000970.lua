...
function c511000970.con(e,tp,eg,ep,ev,re,r,rp)
	return not e:GetHandler():IsStatus(STATUS_CHAINING) and e:GetHandlerPlayer()~=tp
end
function c511000970.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	local race=e:GetLabelObject():GetLabel()
	if chk==0 then return ft>-1 and Duel.CheckReleaseGroupCost(tp,Card.IsRace,1,false,nil,nil,race) end
