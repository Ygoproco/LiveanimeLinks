...
e7:SetCondition(c511002621.con7)
e7:SetTarget(c511002621.tg7)
e7:SetOperation(c511002621.op7)
c:RegisterEffect(e7)
...
e8:SetCondition(c511002621.con8)
e8:SetTarget(c511002621.tg8)
e8:SetOperation(c511002621.op8)
c:RegisterEffect(e8)
...
e9:SetTarget(c511002621.tg9)
e9:SetOperation(c511002621.op9)
c:RegisterEffect(e9)
local e10=e9:Clone()
...
e11:SetTarget(c511002621.tg11)
e11:SetOperation(c511002621.op11)
...
local sg=Duel.GetMatchingGroup(Card.IsCode,tp,0x7f,0x7f,nil,511002621)
...
function c511002621.filter(c,e,tp)
return c:IsFaceup() and c:IsControler(tp) 
end
function c511002621.tg9(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
if chk==0 then return eg:IsExists(c511002621.filter,1,nil,e,tp) end
end

function c511002621.op9(e,tp,eg,ep,ev,re,r,rp,chk)
local g=Duel.GetMatchingGroup(c511002621.filter,tp,LOCATION_MZONE,0,nil,e,tp)
local tc=g:GetFirst()
while  tc do
...
function c511002621.filter4(c,e,tp)
return c:IsFaceup() and c:GetControler()==1-tp 
end
function c511002621.tg11(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
if chk==0 then return eg:IsExists(c511002621.filter4,1,nil,e,tp) end
end

function c511002621.op11(e,tp,eg,ep,ev,re,r,rp,chk)
local g=Duel.GetMatchingGroup(c511002621.filter4,tp,0,LOCATION_MZONE,nil,e,tp)
local tc=g:GetFirst()
while  tc do
tc:SetStatus(STATUS_SUMMON_TURN,false)
tc:SetStatus(STATUS_SPSUMMON_TURN,false)
...
function c511002621.con7(e,tp,eg,ep,ev,re,r,rp)
return eg:IsExists(c511002621.cfilter,1,nil,tp)
end

function c511002621.tg7(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c511002621.op7(e,tp,eg,ep,ev,re,r,rp)
local p,d=Duel.GetChainInfo(0,CHAININFO_TARGET_PLAYER,CHAININFO_TARGET_PARAM)
Duel.Damage(p,d,REASON_EFFECT)
end

--opp
function c511002621.cfilter2(c,tp)
return c:IsReason(REASON_EFFECT) and c:IsPreviousLocation(LOCATION_MZONE) and c:GetPreviousControler()==1-tp
end

function c511002621.con8(e,tp,eg,ep,ev,re,r,rp)
return eg:IsExists(c511002621.cfilter2,1,nil,tp)
end

function c511002621.tg8(e,tp,eg,ep,ev,re,r,rp,chk)
if chk==0 then return true end
local dam=eg:Filter(c511002621.cfilter2,nil,tp):GetSum(Card.GetAttack)/2
Duel.SetTargetPlayer(1-tp)
Duel.SetTargetParam(dam)
end

function c511002621.op8(e,tp,eg,ep,ev,re,r,rp)
local p,d=Duel.GetChainInfo(0,CHAININFO_TARGET_PLAYER,CHAININFO_TARGET_PARAM)
Duel.Damage(p,d,REASON_EFFECT)
end
