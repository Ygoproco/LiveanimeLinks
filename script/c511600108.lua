...
function c511600108.initial_effect(c)
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetTarget(c511600108.target)
	e1:SetOperation(c511600108.operation)
	c:RegisterEffect(e1)
end
function c511600108.filter(c,e,tp,eg,ep,ev,re,r,rp,b)
...
function c511600108.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	e1:SetCondition(c511600108.rtcon)
	e1:SetOperation(c511600108.rtop)
	Duel.RegisterEffect(e1,tp)
end
function c511600108.rtcon(e,tp,eg,ep,ev,re,r,rp)
	local tc=e:GetLabelObject()
	return not (tc:IsControler(1-tp) and tc:IsLocation(LOCATION_GRAVE)) and not tc:IsHasEffect(EFFECT_NECRO_VALLEY)
end
function c511600108.rtop(e,tp,eg,ep,ev,re,r,rp)
	local tc=e:GetLabelObject()
