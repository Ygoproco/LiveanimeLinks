...
function s.lighttg(e,c)
	return bit.band(s[c:GetControler()],ATTRIBUTE_LIGHT)~=0
end
function s.darkcon1(e)
	return bit.band(s[e:GetHandlerPlayer()],ATTRIBUTE_DARK)~=0
end
function s.darkcon2(e)
	return bit.band(s[1-e:GetHandlerPlayer()],ATTRIBUTE_DARK)~=0
end
function s.descon(e,tp,eg,ep,ev,re,r,rp)
	return bit.band(s[Duel.GetTurnPlayer()],ATTRIBUTE_EARTH)~=0 and Duel.GetTurnPlayer()==tp
end
...
function s.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	if bit.band(s[Duel.GetTurnPlayer()],ATTRIBUTE_EARTH)==0 then return end
...
function s.hdcon(e,tp,eg,ep,ev,re,r,rp)
	return bit.band(s[Duel.GetTurnPlayer()],ATTRIBUTE_WATER)~=0
end
...
function s.hdop(e,tp,eg,ep,ev,re,r,rp)
	if not e:GetHandler():IsRelateToEffect(e) then return end
	if bit.band(s[Duel.GetTurnPlayer()],ATTRIBUTE_WATER)==0 then return end
	Duel.DiscardHand(Duel.GetTurnPlayer(),nil,1,1,REASON_EFFECT+REASON_DISCARD)
end
function s.damcon(e,tp,eg,ep,ev,re,r,rp)
	return bit.band(s[Duel.GetTurnPlayer()],ATTRIBUTE_FIRE)~=0
end
...
function s.damop(e,tp,eg,ep,ev,re,r,rp)
	if not e:GetHandler():IsRelateToEffect(e) then return end
	if bit.band(s[Duel.GetTurnPlayer()],ATTRIBUTE_FIRE)==0 then return end
	Duel.Damage(Duel.GetTurnPlayer(),1000,REASON_EFFECT)
end
function s.windcon1(e)
	return bit.band(s[e:GetHandlerPlayer()],ATTRIBUTE_WIND)~=0
end
function s.windcon2(e)
	return bit.band(s[1-e:GetHandlerPlayer()],ATTRIBUTE_WIND)~=0
end
