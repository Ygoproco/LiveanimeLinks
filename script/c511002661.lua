...
function c511002661.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return ct>0 and g:GetCount()==ct and g:FilterCount(aux.MZFilter,nil,tp)+Duel.GetLocationCount(tp,LOCATION_MZONE)>0 end
...
function c511002661.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		e:SetLabel(0)
		return Duel.IsExistingMatchingCard(c511002661.spfilter,tp,LOCATION_HAND+LOCATION_DECK,0,1,nil,e,tp)
	end
