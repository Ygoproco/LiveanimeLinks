...
function c511600120.initial_effect(c)
	--pendulum summon
	...
	e1:SetCountLimit(1)
	e1:SetCondition(c511600120.condition)
	e1:SetTarget(c511600120.target)
	e1:SetOperation(c511600120.operation)
	c:RegisterEffect(e1)
	--destroy replace
	...
	e2:SetTarget(c511600120.reptg)
	e2:SetOperation(c511600120.repop)
	c:RegisterEffect(e2)
end
function c511600120.condition(e,tp,eg,ep,ev,re,r,rp)
...
function c511600120.filter(c)
	return c:IsFaceup() and c:IsSetCard(0x99)
end
function c511600120.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return not e:GetHandler():IsStatus(STATUS_CHAINING) end
	Duel.SetTargetCard(e:GetLabelObject())
end
function c511600120.cfilter(c,e)
	return c:IsOnField() and c:IsType(TYPE_MONSTER) and c:IsRelateToEffect(e) and c:IsRelateToBattle()
end
function c511600120.operation(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local g=Duel.GetChainInfo(0,CHAININFO_TARGET_CARDS):Filter(c511600120.cfilter,nil,e)
...
function c511600120.reptg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
...
function c511600120.repop(e,tp,eg,ep,ev,re,r,rp)
