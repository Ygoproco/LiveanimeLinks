...
function c100000089.filter(c)
	return c:IsFaceup() and c:IsRace(RACE_MACHINE) and (c:IsV() or c:IsW() or c:IsX() or c:IsY() or c:IsZ())
end
function c100000089.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return e:IsHasType(EFFECT_TYPE_ACTIVATE) and Duel.IsExistingTarget(c100000089.filter,tp,LOCATION_MZONE,0,1,nil) end
