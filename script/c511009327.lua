...
function c511009327.filter1(c,e,tp)
	...
	return pg:GetCount()<=1 and c:IsSetCard(0x10db) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and (c:GetRank()>0 or c:IsStatus(STATUS_NO_LEVEL)) 
		and Duel.IsExistingMatchingCard(c511009327.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,c,c:GetRank()+2,pg)
end
function c511009327.filter2(c,e,tp,mc,rk,pg)
...
function c511009327.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c511009327.filter2,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,tc,tc:GetRank()+2,pg)
	local sc=g:GetFirst()
	if sc then
		Duel.BreakEffect()
