...
function c511023006.initial_effect(c)
	...
	e1:SetValue(c511023006.splimit)
	c:RegisterEffect(e1)
	--special summon rule
	...
	e2:SetCondition(c511023006.spcon)
	e2:SetOperation(c511023006.spop)
	c:RegisterEffect(e2)
	--return
...
function c511023006.spfilter(c,code)
	return c:IsAbleToDeckOrExtraAsCost() and c:IsFusionCode(code)
end
function c511023006.spcon(e,c)
	if c==nil then return true end
	local tp=c:GetControler()
	local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
	if ft<-2 then return false end
	local g1=Duel.GetMatchingGroup(c511023006.spfilter,tp,LOCATION_ONFIELD,0,nil,89943723)
	local g2=Duel.GetMatchingGroup(c511023006.spfilter,tp,LOCATION_ONFIELD,0,nil,89621922)
	local g3=Duel.GetMatchingGroup(c511023006.spfilter,tp,LOCATION_ONFIELD,0,nil,80344569)
...
function c511023006.spop(e,tp,eg,ep,ev,re,r,rp,c)
	local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
	local g1=Duel.GetMatchingGroup(c511023006.spfilter,tp,LOCATION_ONFIELD,0,nil,89943723)
	local g2=Duel.GetMatchingGroup(c511023006.spfilter,tp,LOCATION_ONFIELD,0,nil,89621922)
	local g3=Duel.GetMatchingGroup(c511023006.spfilter,tp,LOCATION_ONFIELD,0,nil,80344569)
