	...
	if chk==0 then
		...
		if not res then
			...
			if ce~=nil then
				...
				res=Duel.IsExistingMatchingCard(c100100008.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg3,mf,chkf)
			end
		end
		return res
	end
...
function c100100008.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if ce~=nil then
		...
		mg3=fgroup(ce,e,tp)
		local mf=ce:GetValue()
		sg2=Duel.GetMatchingGroup(c100100008.filter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg3,mf,chkf)
	end
	if sg1:GetCount()>0 or (sg2~=nil and sg2:GetCount()>0) then
		...
		if sg1:IsContains(tc) and (sg2==nil or not sg2:IsContains(tc) or not Duel.SelectYesNo(tp,ce:GetDescription())) then
			...
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
			local gc=mg3:FilterSelect(tp,c100100008.filter3,1,1,nil,tc,mg3,chkf):GetFirst()
