...
function s.discon(e,tp,eg,ep,ev,re,r,rp)
	return not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) and ep~=tp and re:IsActiveType(TYPE_TRAP) 
...
function s.cfilter(c,ft,tp)
	return c:IsSetCard(0x234) and (ft>0 or (c:IsControler(tp) and c:GetSequence()<5)) and c:IsSummonLocation(LOCATION_EXTRA)
end
