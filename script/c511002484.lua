...
function c511002484.checkop(e,tp,eg,ep,ev,re,r,rp)
	local ct=eg:FilterCount(c511002484.cfilter,nil,1-tp)
	if ct<=0 then return end
	for i=1,ct do
		Duel.RegisterFlagEffect(tp,511002484,RESET_PHASE+PHASE_END,0,1)
	end
end
function c511002484.condition(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetFlagEffect(tp,511002484)>0 and Duel.GetTurnPlayer()==tp
...
function c511002484.filter(c)
	Debug.Message(c:GetEffectCount(EFFECT_EXTRA_ATTACK)<Duel.GetFlagEffect(tp,511002484))
	return c:IsFaceup() and (c:GetEffectCount(EFFECT_EXTRA_ATTACK)==0 or c:GetEffectCount(EFFECT_EXTRA_ATTACK)<Duel.GetFlagEffect(tp,511002484))
end
...
function c511002484.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:IsRelateToEffect(e) then
		...
		e1:SetValue(Duel.GetFlagEffect(tp,511002484))
