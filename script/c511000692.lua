...
function c511000692.initial_effect(c)
	...
	e1:SetTarget(c511000692.target)
...
function c511000692.filter(c,tp)
	...
	return c:IsPosition(POS_FACEUP_ATTACK) and Duel.IsExistingMatchingCard(Card.IsPosition,tp,LOCATION_MZONE,0,1,c,POS_FACEUP_ATTACK) 
		and Duel.IsExistingMatchingCard(c511000692.atfilter,tp,0,LOCATION_MZONE,1,c,g)
end
function c511000692.atfilter(c,g)
	return g:IsContains(c)
end
function c511000692.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(tp) and chkc:IsControler(tp) and c511000692.filter(chkc,tp) end
	if chk==0 then return Duel.IsExistingTarget(c511000692.filter,tp,LOCATION_MZONE,0,1,nil,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	Duel.SelectTarget(tp,c511000692.filter,tp,LOCATION_MZONE,0,1,1,nil,tp)
end
function c511000692.activate(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if tc and tc:IsFaceup() and tc:IsRelateToEffect(e) then
		...
		local d=Duel.SelectMatchingCard(tp,c511000692.atfilter,tp,0,LOCATION_MZONE,1,1,nil,ag):GetFirst()
		if d then
