...
function c511009668.initial_effect(c)
	...
	-- Once per turn, if a monster this card points to is targeted for an attack: You can move that monster to another Main Monster Zone; negate the attack.
...
function c511009668.mvcon(e,tp,eg,ep,ev,re,r,rp)
	local lg=e:GetHandler():GetGetLinkedGroup()
	local at=eg:GetFirst()
	return at:IsControler(tp) and lg:IsContains(at)
end
function c511009668.mvcost(e,tp,eg,ep,ev,re,r,rp,chk)
	local at=eg:GetFirst()
	if chk==0 then return not Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 end
