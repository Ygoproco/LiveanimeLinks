...
function c511001367.cfilter(c,e,tp,ft)
	return c:GetLevel()>0 and c:IsAbleToGraveAsCost() and (ft>0 or c:GetSequence()<5) 
		and Duel.IsExistingMatchingCard(c511001367.filter,tp,LOCATION_GRAVE,0,1,nil,c:GetLevel(),e,tp)
end
...
function c511001367.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		...
		e:SetLabel(0)
		return ft>-1 and Duel.IsExistingMatchingCard(c511001367.cfilter,tp,LOCATION_MZONE,0,1,nil,e,tp,ft) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g=Duel.SelectMatchingCard(tp,c511001367.cfilter,tp,LOCATION_MZONE,0,1,1,nil,e,tp,ft)
