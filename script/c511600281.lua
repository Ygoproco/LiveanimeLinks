...
function s.filter1(c,e,tp)
	return c:IsFaceup() and c:IsSetCard(0x119) and Duel.GetLocationCountFromEx(tp,tp,c)>0 
		and Duel.IsExistingMatchingCard(s.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,c)
end
function s.filter2(c,e,tp,mc)
	return mc:IsType(TYPE_LINK,c,SUMMON_TYPE_LINK,tp) and c:IsSetCard(0x119)
		and mc:IsCanBeLinkMaterial(c,tp) and c:IsType(TYPE_LINK) and c:IsCode(mc:GetCode())
