--覇王眷竜 クリアウィング (Anime)
--Supreme King Servant Dragon Clear Wing (Anime)
...
function s.spcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return pg:GetCount()<=0 and eg:IsExists(s.spfilter,1,nil,tp) and Duel.IsExistingMatchingCard(s.cfilter,tp,LOCATION_MZONE,0,1,nil)
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.GetReleaseGroup(tp):Filter(Card.IsSetCard,nil,0x20f8)
	local pg=aux.GetMustBeMaterialGroup(tp,Group.CreateGroup(),tp,nil,nil,REASON_SYNCHRO)
	if pg:GetCount()<=0 and g:GetCount()>1 and aux.SelectUnselectGroup(g,e,tp,2,2,s.rescon,0) and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_SYNCHRO,tp,false,true) 
