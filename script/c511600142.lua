...
function c511600142.initial_effect(c)
	--disable
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(511600142,0))
	e1:SetCategory(CATEGORY_DISABLE)
	e1:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_O)
	e1:SetCode(EVENT_ATTACK_ANNOUNCE)
	e1:SetRange(LOCATION_GRAVE)
	e1:SetCost(aux.bfgcost)
	e1:SetCondition(c511600142.atkcon)
	e1:SetTarget(c511600142.atktg)
	e1:SetOperation(c511600142.atkop)
	c:RegisterEffect(e1)
end
function c511600142.atkcon(e,tp,eg,ep,ev,re,r,rp)
...
function c511600142.atktg(e,tp,eg,ep,ev,re,r,rp,chk)
	local tc=e:GetLabelObject()
	if chk==0 then return tc:IsOnField() end
	Duel.SetTargetCard(tc)
end
function c511600142.atkop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and aux.disfilter1(tc) and tc:IsControler(1-tp) then
		...
		e4:SetValue(tp)
		e4:SetCondition(c511600142.rdcon)
		e4:SetOperation(c511600142.rdop)
		e4:SetReset(RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END)
		tc:RegisterEffect(e4)
	end
end
function c511600142.rdcon(e,tp,eg,ep,ev,re,r,rp)
